...
import { AppState, Message, Appointment, Transaction, Habit, Task } from './types';
...
const App: React.FC = () => {
  ...
  const [state, setState] = useState<AppState>(() => {
    const saved = localStorage.getItem('secretary_state_v7');
    if (saved) {
    ...
    return {
      ...
      tasks: [],
      messages: [{ id: '1', role: 'model', text: 'Pronto para começar. Como posso ajudar com sua agenda, finanças ou saúde agora?', timestamp: new Date() }]
    };
  });
  ...
  useEffect(() => {
    localStorage.setItem('secretary_state_v7', JSON.stringify(state));
  }, [state]);
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    // Captura o histórico atual ANTES de adicionar a nova mensagem
    const snapshotHistory = [...state.messages];
    ...
    const handleTool = (name: string, args: any) => {
      setState(prev => {
        ...
        if (name === 'add_task') {
          ...
          newState.habits = prev.habits.map(h => h.name.toLowerCase() === args.name.toLowerCase() ? { ...h, current: h.current + (Number(args.amount) || 0), lastUpdated: new Date().toISOString() } : h);
          ...
          newState.transactions = [...prev.transactions, { id: Math.random().toString(36).substr(2, 9), amount: Number(args.amount) || 0, type: args.type, category: args.category, date: new Date().toISOString(), description: args.description || '' }];
        }
        return newState;
      });
    };

    // Chamada à API com o snapshot garantido
    const aiResponseText = await getSecretaryResponse(currentInput, { ...state, messages: snapshotHistory }, handleTool);
  ...
  return (
    <div className="flex h-screen bg-gray-100 overflow-hidden flex-col md:flex-row">
      <div className="w-full md:w-20 bg-emerald-950 flex md:flex-col items-center justify-around md:justify-start md:py-6 md:space-y-8 shadow-2xl z-20 order-2 md:order-1 h-16 md:h-full">
        <button onClick={() => setActiveTab('chat')} className={`p-3 rounded-xl transition-all ${activeTab === 'chat' ? 'bg-white/20 text-white shadow-lg' : 'text-emerald-200 hover:text-white'}`}><MessageSquare size={24} /></button>
        <button onClick={() => setActiveTab('dashboard')} className={`p-3 rounded-xl transition-all ${activeTab === 'dashboard' ? 'bg-white/20 text-white shadow-lg' : 'text-emerald-200 hover:text-white'}`}><LayoutDashboard size={24} /></button>
        ...
        <header className="bg-emerald-800 text-white p-4 shadow-lg flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-emerald-300 text-[10px] uppercase font-bold tracking-widest">Saúde & Gestão</p>
            </div>
          </div>
          <div className="flex space-x-2">
            <button onClick={() => { localStorage.clear(); window.location.reload(); }} className="p-2 text-emerald-300 hover:text-white transition-colors" title="Resetar Memória"><RefreshCw size={18} /></button>
          </div>
        </header>
          ...
          <>
            <main className="flex-1 overflow-y-auto whatsapp-bg p-4 space-y-4">
              {state.messages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in`}>
                  <div className={`max-w-[85%] p-3 rounded-2xl shadow-md ${msg.role === 'user' ? 'bg-[#dcf8c6] rounded-tr-none' : 'bg-white rounded-tl-none'}`}>
                    <p className="text-gray-800 text-sm md:text-base whitespace-pre-wrap leading-relaxed">{msg.text}</p>
                    <div className="flex justify-end items-center mt-1"><span className="text-[10px] text-gray-500 font-medium">{msg.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span></div>
                  </div>
                </div>
              ))}
            ...
            <footer className="bg-[#f0f2f5] p-4 flex border-t border-gray-200">
              <form className="flex-1 flex space-x-3" onSubmit={handleSendMessage}>
                <input type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder="Como posso ajudar hoje?" className="flex-1 p-3.5 px-6 rounded-full outline-none bg-white text-sm shadow-inner border border-gray-200 focus:border-emerald-400" />
                <button type="submit" className="p-3.5 rounded-full bg-emerald-700 text-white shadow-lg hover:bg-emerald-800 transition-all"><Send size={22} /></button>
              </form>
            </footer>
          </>
        ) : (
